...
contains(HOST, i686-pc-linux-gnu) {
...
else:contains(HOST, x86_64-pc-linux-gnu) {
    message(Building with SSE2 support)
    QMAKE_CXXFLAGS += -msse2
    QMAKE_CFLAGS += -msse2
...
else:contains(HOST, x86_64-apple-darwin) {
    message(Building with SSE2 support)
    QMAKE_CXXFLAGS += -msse2
    QMAKE_CFLAGS += -msse2
